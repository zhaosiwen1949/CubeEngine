...
vec3 faceCoordsToXYZ(int i, int j, int faceID, int faceSize)
{
	...
	if (faceID == 0) return vec3(-1.0f, A - 1.0f, B - 1.0f); // -X +Z 4
	if (faceID == 1) return vec3(A - 1.0f, -1.0f, 1.0f - B); // -Y +X 2
	if (faceID == 2) return vec3(1.0f, A - 1.0f, 1.0f - B); // +X -Z 5
	if (faceID == 3) return vec3(1.0f - A, 1.0f, 1.0f - B); // +Y -X 0
	if (faceID == 4) return vec3(B - 1.0f, A - 1.0f, 1.0f); // +Z +Y 3
	if (faceID == 5) return vec3(1.0f - B, A - 1.0f, -1.0f); // -Z -Y 1
...
Bitmap convertEquirectangularMapToVerticalCross(const Bitmap& b)
{
	...
	{
		ivec2(faceSize, faceSize * 3), // -X +Z
		ivec2(0, faceSize), // -Y +X
		ivec2(faceSize, faceSize), // +X -Z
		ivec2(faceSize * 2, faceSize), // +Y -X
		ivec2(faceSize, 0), // +Z +Y
		ivec2(faceSize, faceSize * 2) // -Z -Y
	};
