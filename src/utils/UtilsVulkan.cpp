...
VkResult createSwapchain(VkDevice device, VkPhysicalDevice physicalDevice, VkSurfaceKHR surface, uint32_t graphicsFamily, uint32_t* width, uint32_t* height, VkSwapchainKHR* swapchain, bool supportScreenshots)
{
    ...
    VkExtent2D extent2D = swapchainSupport.capabilities.currentExtent;
    // 更新 framebufferWidth、framebufferHeight
...
VkResult createFence(VkDevice device, VkFence* outFence)
{
    const VkFenceCreateInfo ci = {
            .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO,
            .flags = VK_FENCE_CREATE_SIGNALED_BIT,
    };
    return vkCreateFence(device, &ci, nullptr, outFence);
}
...
{
    // 由于 MACOS 的视网膜屏的情况，所以需要根据 SwapChain 中的格式更新这里的大小，故更新函数迁入 createSwapchain 中
...
//	vkDev.framebufferHeight = height;
	...
	VK_CHECK(createSwapchain(vkDev.device, vkDev.physicalDevice, vk.surface, vkDev.graphicsFamily, &vkDev.framebufferWidth, &vkDev.framebufferHeight, &vkDev.swapchain));
...
//	VK_CHECK(vkGetBestComputeQueue(vkDev.physicalDevice, &vkDev.computeFamily));
	...
	VK_CHECK(createSwapchain(vkDev.device, vkDev.physicalDevice, vk.surface, vkDev.graphicsFamily, &vkDev.framebufferWidth, &vkDev.framebufferHeight, &vkDev.swapchain, supportScreenshots));
	const size_t imageCount = createSwapchainImages(vkDev.device, vkDev.swapchain, vkDev.swapchainImages, vkDev.swapchainImageViews);
	vkDev.commandBuffers.resize(imageCount);

	VK_CHECK(createSemaphore(vkDev.device, &vkDev.semaphore));
	VK_CHECK(createSemaphore(vkDev.device, &vkDev.renderSemaphore));
	VK_CHECK(createFence(vkDev.device, &vkDev.fence));
...
void destroyVulkanRenderDevice(VulkanRenderDevice& vkDev)
{
	...
	vkDestroyFence(vkDev.device, vkDev.fence, nullptr);
...
bool createPipelineLayoutWithConstants(VkDevice device, VkDescriptorSetLayout dsLayout, VkPipelineLayout* pipelineLayout, uint32_t vtxConstSize, uint32_t fragConstSize)
{
	...
	const VkPipelineLayoutCreateInfo pipelineLayoutInfo = {
		...
		.pNext = nullptr,
		.flags = 0,
		.setLayoutCount = (dsLayout == nullptr) ? 0u : 1u,
		.pSetLayouts = (dsLayout == nullptr) ? nullptr : &dsLayout,
