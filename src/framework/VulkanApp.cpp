...
Resolution detectResolution(int width, int height)
{
	...
	const uint32_t windowW = (width  > 0 ? width : (uint32_t)(info->width * width / -100)) * factor;
	const uint32_t windowH = (height > 0 ? height : (uint32_t)(info->height * height / -100)) * factor;
...
bool drawFrame(VulkanRenderDevice& vkDev, VulkanInstance& vk, const std::function<void(uint32_t)>& updateBuffersFunc, const std::function<void(VkCommandBuffer, uint32_t)>& composeFrameFunc)
{
	vkWaitForFences(vkDev.device, 1, &vkDev.fence, VK_TRUE, UINT64_MAX);
	...
	vkResetFences(vkDev.device, 1, &vkDev.fence);

	VK_CHECK(vkQueueSubmit(vkDev.graphicsQueue, 1, &si, vkDev.fence));
...
void VulkanApp::mainLoop()
{
	...
	do
	{
		...
		bool frameRendered = drawFrame(ctx_.vkDev, ctx_.vk,
