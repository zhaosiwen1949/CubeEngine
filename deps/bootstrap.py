...
def main(argv):
    ...
    # 读取 bootstrap.json 的内容
    data = readJSONData(bootstrap_filename)
    if data is None:
        return -1;

    # some sanity checking
    # 检查 bootstrap.json 的内容是否合法
    for library in data:
    ...
    # 通过命令行指定 bootstrap.json，并与默认 bootstrap.json 文件中的数据合并
    local_data = None
        ...
        try:
            ...
            if source is not None:
                ...
                if src_type == "sourcefile":
                    ...
                    # 对于 archive 类型的库，直接下载到 archive/${name} 文件夹中
                    sha1 = source.get('sha1', None)
                    user_agent = source.get('user-agent', None)
                    try:
                        if force_fallback:
                    ...
                    # 对于 git 类型的库，直接 clone 下来，放在 src/${name}
